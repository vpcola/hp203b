...
static esp_err_t hp203b_start_convert(i2c_port_t i2c_num)
{
    ...
    ret = hp203b_write_cmd(i2c_num, HP203B_DEFAULT_ADDRESS, (uint8_t *) &command, 1);
...
static esp_err_t hp203b_get_sensor_data(i2c_port_t i2c_num, uint8_t sensorcmd, float * value)
{
    ...
    hp203b_write_cmd(i2c_num, HP203B_DEFAULT_ADDRESS, (uint8_t *) &sensorcmd, 1);

    /* Now read a 3 byte value from the NVram registers */
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    size_t size = 3;
    i2c_master_write_byte(cmd, ( HP203B_DEFAULT_ADDRESS << 1) | READ_BIT, ACK_CHECK_EN);
    if (size > 1) {
...
esp_err_t hp203b_init(i2c_port_t i2cnum)
{
    ...
    return hp203b_write_cmd(i2cnum, HP203B_DEFAULT_ADDRESS, (uint8_t *) &command, 1);
}
